...
install.packages(c("dplyr", "tidytext", "topicmodels", "tm", "zoo"))
...
rare_words <- data.frame(
  word = names(word_freq[word_freq <= 5]),
...
crp <- aggregate(
  pap$word,
  by = list(pap$paragraph),
  FUN = paste0,
  collapse = " "
)
names(crp) <- c("paragraph", "text")
s <- SimpleCorpus(VectorSource(setNames(crp$text, crp$paragraph)))
...
lda_fit <- LDA(m, k = K, method = "Gibbs", control)
@
...
<<eval=FALSE>>=
library(uuml)
lls <- extract_log_liks(lda_fit)
theta <- extract_theta(lda_fit)
phi <- extract_phi(lda_fit)
@
